...
# ---------------------------------------------------------
# 2. SETUP VIDEO PATHS (Sabhi Videos ke liye)
# ---------------------------------------------------------
# Ye code automatically dhoond lega ki video kahan rakhi hain
...
# --- STAGE 2: Success ---
    ...
    # Ab yahan hum teeno video use kar sakte hain
    # (Filhal vdo3 chala rahe hain, aap vdo1_path ya vdo2_path bhi likh sakte ho)
    
    if os.path.exists(vdo3_path):
        st.video(vdo3_path, format="video/mp4", start_time=0)
    else:
        st.error(f"Video not found: {vdo3_path}")
        st.image("https://media.tenor.com/26BRv0ThflsHCqDrG/giphy.gif")
    
    # Agar aapko vdo1 chalana hai to upar "vdo3_path" ko hatakar "vdo1_path" likh dena
